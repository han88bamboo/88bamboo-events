...
- [x] Homepage widget JS + embed snippet for the Shopify theme. `frontend/public/widget/events-widget.js` (served under basePath at `/a/events/widget/events-widget.js`) — standalone vanilla JS, no iframe, scoped/namespaced styles, `data-*` config, fetches `GET /events/widget` cross-origin from the API and links cards to the apex detail pages. Embed snippet + local-test snippet in `frontend/public/widget/README.md`.
  ...
  **Round 5 — CHOSEN designs / discovered sub-tasks:**
  ...
  - **Magic-link "~3 uses" interpretation:** gate on EXPIRY, not a hard first-use lock — GET (`/context`, scanner pre-clicks) never consumes; `used_at` is stamped for audit on the first committed edit. Within the 30-min window the link tolerates the handful of hits it's meant to survive.
  - **Pre-approval edit request UX (noted):** `/edits/request-link` + the `manage` page identify an event by its **slug**, which a still-pending event does not have. The pre-approval edit *mechanism* is identical (same `create_magic_link` helper, proven end-to-end below), but a public handle for a slug-less pending event — e.g. an edit link embedded in the "under review" confirmation email, or matching by email+name — is a small deferred follow-up. Post-approval editing (the headlined requirement) is fully reachable from the public detail page.
  ...
  **Heads-up notes carried from Phases 3–4 (read before starting Phase 5):**
  ...
  - **Magic links are cookie-free by design** (`magic_links` table exists: store `token_hash` only, 30-min expiry, tolerate ~3 uses for email pre-scanners, fresh link per edit). Editing may run through the App Proxy, which strips cookies — so the edit session is carried by the URL token, never a cookie (plan §4/§7).
...
- [x] Magic-link edit both pre- and post-approval — proven in the Round-5 local proof (post-approval: repoint + keep slug, no capture; pre-approval: new pending version with the hold moved onto it + old version superseded).
...
- **Pre-approval edit request UX — owner decision needed (Phase 5 follow-up).** The public `manage` page + `/edits/request-link` identify a listing by its **slug**, which a still-`pending_review` event doesn't have yet, so a submitter can't self-serve a pre-approval edit link from the site. The pre-approval edit *mechanism* is built and proven (same `create_magic_link` helper; the hold moves onto the new version and the old one is superseded), so this is only about how a slug-less pending submitter GETS the link. Options: (a) embed a fresh edit link in the "under review" confirmation email (touches `scripts/submissions.py`); (b) let `request-link` match a pending event by submitter-email + event-name; (c) leave it — pre-approval submitters just resubmit. Post-approval editing (the headlined requirement) is fully reachable from the public detail page and needs no change. Pick one before go-live if pre-approval self-edit matters.
