...
- **Pre-approval edit request UX — RESOLVED (was: owner decision).** Implemented option (a): the "under review" confirmation email now includes a fresh 30-min pre-approval edit link (`scripts/submissions.py` mints it in the persist transaction; `notifications.send_under_review` carries the URL), so a slug-less pending submitter can self-serve an edit. Verified end-to-end this session. (Options (b) match-by-email+name and (c) leave-it were not needed.)
