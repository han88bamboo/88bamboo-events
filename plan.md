...
- [~] Server-side session check on approve / reject / capture / unpublish (the carve-out). approve + reject are built and GUARDED server-side; **capture is folded into approve**; **unpublish is 4B** (the guard helper `admin_auth.admin_required` is reusable for it). Proven: `/admin/pending|approve|reject` return 401 without/with a tampered token; the money/listing writes never run unless the HMAC-signed token verifies.
...
- [ ] Live listings, unpublish, past events, version history, pricing-tier CRUD, analytics incl. expiring-soon countdown  *(4B)*
- [ ] Hourly auto-release job + daily digest + 48h/24h alerts (APScheduler)  *(4B)*
- [~] All transactional emails wired (local mailer). 4A added approval / rejection (with reason) / re-pay emails (`notifications.py`, §8 wording) on top of the two Phase-3 emails. Magic-link + edit emails are Phase 5.
