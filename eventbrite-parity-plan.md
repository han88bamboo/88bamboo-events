...
- [ ] (Optional stretch, DEFERRED) admin panels show a mini-map / coords for review. *(Not needed for parity; the admin edit form already carries coords.)*
...
- [x] **Docker/env wiring.** `NEXT_PUBLIC_MAPS_BROWSER_KEY` added to `docker-compose.yml` `events-web` (mirrors the Stripe key). Already in `frontend/.env.local.example` (owner).
  ...
  **Owner deploy note (prod DB — run once, before/with the Phase-7 deploy):**
  - Apply the hand-written migration (idempotent, `IF NOT EXISTS` + `ON CONFLICT DO NOTHING`, wrapped in a transaction):
...
- **[OPEN — owner action]** Run `psql -f database/migrations/ep2-location.sql` on the **live** DB before the Phase-7 deploy; sanity-check the large region seed lists (Russia/Indonesia) — they're DB-editable.
