...
| F1 | **Optional magic-link login on the submit page** — reuse the existing account magic link so a submitter can authenticate their email; unlocks the organiser-name field | M | EP-7 |
| F2 | **Public event-organiser name** — an optional publicly-shown organiser name (typed or picked from the submitter's previously-used names) on a NEW `event_organiser_names` registry, first-come-first-served + cross-account-unique | M | EP-7 |
| F3 | **Organiser display** — show "Organised by …" on the detail page + `organizer` in JSON-LD (+ admin display) | S | EP-7 |
...
- [ ] **Prod.** Apply `ep6-occurrences.sql` to the live `drinkxprod`/`events` DB via the bastion (additive/safe); take a `pg_dump` backup first, as in EP-2. **(owner — one new table + index, no backfill.)**
  ...
  **Owner-verification checklist (EP-6)** — on the running docker stack (`docker compose down -v && docker compose up --build` to re-seed with the new table; Stripe test key + `stripe listen` + `NEXT_PUBLIC_MAPS_BROWSER_KEY`), browse `http://localhost:8080/a/events`:
  ...
  - **Single-date "Ends" smart-fills from "Starts" (refinement 2026-07-08).** In the `!multi` branch only, "Ends" auto-follows "Starts" (the same `YYYY-MM-DDTHH:MM` string verbatim — zero duration, no default offset) so the submitter just nudges the end time instead of re-typing the whole date. It is a **one-time smart default, not a permanent mirror**: a `useRef(Boolean(values.end_datetime))` flag ("has Ends been taken over?") flips `true` the instant the user edits "Ends", after which "Starts" changes never overwrite it — and because the edit surfaces mount with "Ends" already prefilled, the flag starts `true` there so no auto-fill ever fires on an edit. While the flag is still `false` (fresh submit, Ends untouched) *every* "Starts" change keeps mirroring into "Ends". The **multi-date table is untouched** — it keeps its independent per-row `{date,start,end}` parts (81b8b73). Presentational client-side prefill only; `submission_validation.py` untouched.
...
### Phase EP-7 — Submitter login + public organiser name (F1, F2, F3) — the third schema phase

> Added 2026-07-08. Adds an **optional magic-link login on the "List an event" page** and, once
> logged in, an **optional publicly-shown event-organiser name** (typed, or picked from the
> submitter's previously-used names) with **cross-account uniqueness** (first-come-first-served).
> Same additive/nullable/no-backfill discipline as EP-2/EP-6. **Not yet started** — this section is
> the build spec; a fresh session implements it. (No EP-5; owner's numbering.)

**Motivation (owner, 2026-07-08):** the submit form collects a *public contact email* but no
organiser identity — a listing can't say who is running it. Eventbrite shows an organiser name.
Owner wants an organiser name that is (a) publicly shown, (b) reusable across that organiser's own
events, and (c) protected so a different person can't submit under someone else's organiser name.

**Design decisions (RESOLVED with owner 2026-07-08):**
- **F-D1 — Login is OPTIONAL and only unlocks the organiser-name field.** The submit page keeps
  working for anonymous submitters exactly as today; magic-link login is an *opt-in* control on the
  wizard's **Details** step. Anonymous submissions simply carry no organiser name. **Reuse the
  existing account magic-link** (email-scoped, `event_id` NULL) — `create_account_link` /
  `resolve_account_token` in [`magic_links.py`](backend/magic_links.py), the
  `/account/request-link` endpoint + `send_account_link` in [`account.py`](backend/scripts/account.py) —
  **do not build a second auth system.** The only new piece is letting the emailed link **return to
  the submit page** (`/submit?token=…`) instead of only `/my-events`, and a small read that resolves
  the token → the authenticated email + that email's past organiser names.
- **F-D2 — The logged-in email IS `submitter_email`.** Once authenticated, the submit form's
  `submitter_email` field is **prefilled with the logged-in address and made read-only** (identity =
  the authenticated email). This is what makes the ownership check trustworthy — a typed
  `submitter_email` can't be trusted. The **public contact email stays a separate, editable,
  optional field** (unchanged). Ownership is keyed to the **submitter/logged-in email, NEVER the
  public contact email.**
- **F-D3 — Ownership lives in a NEW `event_organiser_names` registry table** (not derived ad-hoc):
  `normalised_name` **UNIQUE** + `owner_email` + `display_name` + `created_at`. The UNIQUE constraint
  gives race-proof **first-come-first-served**: the first email to claim a name owns it; that owner
  may reuse it on any number of their events; a different email is rejected. This table is also the
  source for the submitter's **"my previous organiser names" dropdown**.
- **F-D4 — Duplicate matching = case-insensitive + trimmed + normalised** (lowercase, collapse
  internal whitespace, strip punctuation, fold accents) into `normalised_name`; the `display_name`
  keeps the submitter's original casing for display. So `"Sake Matsuri Singapore"`,
  `"sake  matsuri singapore "`, and `"Saké-Matsuri, Singapore"` all collide.
- **F-D5 — Organiser name is server-gated on auth + optional.** The submit endpoints accept an
  optional account token; an `organiser_name` is only honoured when the request carries a **valid**
  token (else ignored). The claim/conflict check runs **before the payment hold (3a)** AND is
  re-checked + committed inside the **3b persist transaction** (the UNIQUE constraint catches a race →
  rollback + cancel the intent, mirroring the EP-6 "authorise-then-persist, roll back on failure").
- **F-D6 — Backward compat = additive, nullable, no backfill** (mirrors D-4/E-D2). New
  `event_versions.organiser_name` column is nullable; legacy events have none and render with no
  "Organised by" line. The registry starts empty; existing events don't retroactively claim names.

**Ripple map (EP-7):**

| Layer | File | EP-7 change |
|---|---|---|
| DB schema | [`database/schema.sql`](database/schema.sql) | **NEW `event_organiser_names`** (`normalised_name` UNIQUE, `owner_email`, `display_name`, `created_at`) + index on `owner_email`; **NEW nullable `event_versions.organiser_name`** (display string, snapshotted per version). |
| DB migration | `database/migrations/ep7-organiser.sql` (NEW) | additive/idempotent: `CREATE TABLE event_organiser_names` + `ALTER TABLE event_versions ADD COLUMN organiser_name`. No backfill. Hand-apply to prod via the bastion. |
| Auth reuse | [`backend/magic_links.py`](backend/magic_links.py) | reuse `create_account_link` / `resolve_account_token` as-is (no new token type). |
| Login link | [`scripts/account.py`](backend/scripts/account.py) | let `/account/request-link` (or a thin sibling) emit a link that returns to `/submit?token=…`; **NEW read** (e.g. `/account/organisers` or fold into a submit-context) → `{ email, organiser_names:[…] }` for the prefill + dropdown, resolved from the account token. |
| Organiser helpers | `backend/organiser_names.py` (NEW) | `normalise_organiser_name(s)` (the F-D4 rule) + `claim_organiser_name(cursor, name, owner_email)` (SELECT owner by `normalised_name`; conflict → error; else upsert the claim). Kept separate so `submission_validation` stays DB-free. |
| Validators | [`submission_validation.py`](backend/submission_validation.py) | accept `organiser_name` (length cap, produce `cleaned["organiser_name"]`); the DB uniqueness **claim** is done by the persist layer (like the geo list is loaded outside), gated on the authenticated email. |
| Submit persist | [`scripts/submissions.py`](backend/scripts/submissions.py) | 3a: resolve the optional account token → auth email; force `submitter_email` = auth email; if `organiser_name` set, run the conflict check (400 if taken by another owner) BEFORE the hold. 3b: re-check + `claim_organiser_name` + INSERT `organiser_name` on the version, all in the txn. |
| Edit versioning | [`event_versioning.py`](backend/event_versioning.py) | INSERT `organiser_name` on the new version (carry forward when unchanged, like coords/image); re-run the claim when it changed (owner is the event's own submitter email). |
| Edit context | [`scripts/edits.py`](backend/scripts/edits.py) + [`account.py`](backend/scripts/account.py) + [`admin.py`](backend/scripts/admin.py) | `/context` returns `organiser_name` (+ the owner's past names for the dropdown) so the edit form prefills. |
| Public read | [`scripts/events.py`](backend/scripts/events.py) | add `organiser_name` to `_PUBLIC_COLUMNS` (listing + detail + widget inherit it). |
| Submit form | [`SubmitEvent.js`](frontend/components/views/landingPages/SubmitEvent/SubmitEvent.js) | Details step: an **optional login panel** (request link → email → return via `?token=`); when authed, `submitter_email` prefilled + read-only, an **`organiser_name`** input with a **datalist/dropdown** of the user's past names; anonymous = field hidden. |
| Edit form | [`EditEvent.js`](frontend/components/views/publicPages/EditEvent/EditEvent.js) | organiser-name field (the edit session already proves ownership via its magic link). |
| Admin edit | [`AdminEditModal.js`](frontend/components/views/admin/AdminEditModal/AdminEditModal.js) | `buildContext` passes `organiser_name`. |
| Detail page | [`EventDetail.js`](frontend/components/views/publicPages/EventDetail/EventDetail.js) | render **"Organised by &lt;name&gt;"** when present. |
| SEO JSON-LD | [`core/utils/seo.js`](frontend/core/utils/seo.js) | add `organizer` (`Organization`/`Person` with `name`) when present. |
| Admin display | [`ReviewQueue.js`](frontend/components/views/admin/ReviewQueue/ReviewQueue.js) / [`LiveListings.js`](frontend/components/views/admin/LiveListings/LiveListings.js) | show the organiser name (+ its owner email for the reviewer). |

**Checklist:**
- [ ] **Schema + migration.** `event_organiser_names` (`normalised_name` UNIQUE + `owner_email` + `display_name`) + index; nullable `event_versions.organiser_name`. NEW idempotent `database/migrations/ep7-organiser.sql` (additive, no backfill). Local re-seed via `docker compose down -v`.
- [ ] **Auth reuse + login link.** Reuse the account magic link; add a submit-page return path (`/submit?token=…`) and a read that resolves the token → `{ email, organiser_names:[…] }`. Anti-enumeration + rate-limit unchanged.
- [ ] **Organiser helpers.** NEW `organiser_names.py`: `normalise_organiser_name` (case-insensitive + trim + normalise punctuation/accents/whitespace) + `claim_organiser_name` (first-come-first-served conflict + upsert). **Unit tests** (normalisation cases + conflict-vs-own-reuse-vs-new).
- [ ] **Validator.** `validate_submission` accepts + length-caps `organiser_name` and returns `cleaned["organiser_name"]` (format only; the DB claim is the persist layer's job). Unchanged for callers that omit it.
- [ ] **Persist — submit.** Resolve the optional account token → auth email; force `submitter_email` = auth email; conflict-check the organiser name BEFORE the 3a hold; re-check + claim + INSERT on the version inside the 3b transaction (UNIQUE catches the race → rollback + cancel intent).
- [ ] **Persist — edits.** `create_edit_version` INSERTs `organiser_name`; re-runs the claim when it changed (owner = the event's submitter email).
- [ ] **Serialise.** `/context` (edits/account/admin) returns `organiser_name` + the owner's past names; `_PUBLIC_COLUMNS` in `events.py` adds `organiser_name`.
- [ ] **Forms.** Submit Details step: optional login panel + prefilled read-only `submitter_email` + `organiser_name` input with a past-names datalist; `EditEvent` + `AdminEditModal` organiser field. Anonymous submit unchanged.
- [ ] **Detail page + JSON-LD.** "Organised by …" on `EventDetail`; `organizer` in `seo.js`.
- [ ] **Admin display.** ReviewQueue + LiveListings show the organiser name (+ owner email).
- [ ] **Regression + tests.** Anonymous submit unchanged; logged-in submit locks `submitter_email` to the auth email; a different email is blocked from a taken organiser name; the owner reuses their own name freely; legacy events render with no organiser line. **Backend suite green** (add normalisation + claim/conflict tests); `next build` clean.
- [ ] **Prod.** Apply `ep7-organiser.sql` to the live `drinkxprod`/`events` DB via the bastion (additive/safe); `pg_dump` backup first, as in EP-2/EP-6.

  **Owner-verification checklist (EP-7)** — _(fill in when built; mirror the EP-2/EP-6 style: anonymous submit unchanged; magic-link login on the submit page prefills + locks the submitter email; organiser-name field appears with the past-names dropdown; a second email is blocked from a name already claimed; the owner reuses their own name; the detail page shows "Organised by …"; edits keep the name; legacy events still render)._

  Round notes (EP-7): _(fill in when built)_
...
- **[RESOLVED 2026-07-08 — EP-7 scope]** Owner added **submitter login + a public event-organiser name** as a new **EP-7** (spec written above; **not yet implemented**). Decisions: **optional magic-link login on the submit page, reusing the existing account magic link, that only unlocks the organiser-name field** (anonymous submits unchanged) (F-D1); **the logged-in email becomes the read-only `submitter_email`**, ownership keyed to it, never the public contact email (F-D2); **a new `event_organiser_names` registry table (`normalised_name` UNIQUE + `owner_email`) for first-come-first-served, cross-account-unique claims** + the submitter's past-names dropdown (F-D3); **duplicate matching is case-insensitive + trimmed + normalised** for punctuation/accents/whitespace (F-D4); **organiser name is server-gated on a valid account token, checked before the 3a hold + committed in the 3b txn** (F-D5); **additive/nullable/no-backfill** (F-D6).
...
- **2026-07-08 — EP-6 refinement: single-date "Ends" auto-fills from "Starts".** UX gap in [`ScheduleFields`](frontend/components/common/ScheduleFields/ScheduleFields.js): the single-date (`!multi`) "Starts"/"Ends" pair was fully independent, so a submitter had to re-type the whole date into "Ends" even though it almost always matches "Starts". Fix: "Ends" now auto-follows "Starts" (same `YYYY-MM-DDTHH:MM` verbatim, zero duration) as a **one-time smart default** — a `endOwnedRef = useRef(Boolean(values.end_datetime))` flag gates it; while unowned, each "Starts" change also patches "Ends"; the ref flips `true` the moment the user edits "Ends" (or, on the three edit surfaces, starts `true` because "Ends" mounts prefilled), after which "Starts" changes never clobber it. **Multi-date table untouched** (independent per-row parts, 81b8b73). **No backend change** (presentational prefill only; `submission_validation.py` untouched). **Verified:** `next build` clean; browser-walked on `/a/events/submit` — mirror-on-first-set, keeps-mirroring-until-Ends-edited, edited-Ends-sticks-with-no-re-clobber, and the multi-date per-row independence all confirmed; console clean. Edit-surface case (#3) is safe by construction (ref inits `true` when Ends is prefilled) — not exercised live (needs the backend + a token). Committing pending owner go-ahead.
- **2026-07-08 — EP-7 scoped (plan only, no code).** Owner flagged that the submit form has a public *contact email* but no **organiser identity**. Added **EP-7 — submitter login + public organiser name**: an *optional* magic-link login on the "List an event" page (reusing the existing account magic link — `create_account_link`/`resolve_account_token`, `/account/request-link`, `send_account_link` — NOT a second auth system) that unlocks an optional, publicly-shown **organiser name**. Locked six decisions with the owner (F-D1…F-D6, §2/§5): optional login that only unlocks the field (anonymous submits unchanged); the logged-in email becomes the read-only `submitter_email` and is the ownership key (never the public contact email); a NEW `event_organiser_names` registry table (`normalised_name` UNIQUE + `owner_email`) for race-proof first-come-first-served + the past-names dropdown; case-insensitive+trimmed+normalised (punctuation/accents/whitespace) duplicate matching; organiser name server-gated on a valid account token, conflict-checked before the 3a hold and committed in the 3b txn; additive/nullable/no-backfill. Wrote the full "Phase EP-7" build spec (motivation + decisions + ripple map + checklist) above. **No code written this session** (plan-only task).
