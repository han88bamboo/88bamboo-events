...
- **[RESOLVED 2026-07-06]** `ep2-location.sql` applied to the live `drinkxprod`/`events` RDS via the bastion (see the deploy note above); verified. **[OPEN — still worth doing]** sanity-check the large region seed lists (Russia/Indonesia) — they're DB-editable.
